...
pip install --upgrade tensorflow-probabilityPrerequisites
-------------
tensorflow >=2.0.0a0
...
# this two lib must be imported before tf and tl, ohterweise there will be error
import sklearn
import cv2
...
if __name__ == '__main__':
    ...
    if args.test:
        ...
        for episode in range(TEST_EPISODES):
            state, _ = env.reset(options={"x_init": np.pi/2, "y_init": 5.0})
            episode_reward = 0
            for step in range(MAX_STEPS):
                env.render()
